...
function toogleBlock(showingBlockIndex /*Индекс DOM элемента в коллекции $(".block_ins")*/, duration, onComplete, isRemove) {
    ...
    var isCurrentElementLast = currentBlockIndex == blocksCollection.length - 1;
    var isShowingElementLast = showingBlockIndex == blocksCollection.length - 1;

    console.log('isLastShowingElement: ' + isShowingElementLast);

    if (showingBlockIndex != -1 && showingBlockIndex > currentBlockIndex && !isCurrentElementLast) {
